...
* @returns String
*/
export const createWhere = (conditions: any) => {
...
export const update = ({ table, data, where }: { table: string, data: any, where: object }) => `UPDATE ${table} SET ${Object.keys(data).map(key => `${key}=${typeof data[key] === "string" ? `'${data[key].replace(/\n/g, "\\n").replace(/\r/g, "\\n").replace(/'/g, "\\'").trim()}'` : data[key] === undefined ? null : data[key]}`).join(",")} ${createWhere(where)}`
